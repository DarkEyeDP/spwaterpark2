...
function WeatherConditions({ current }: { current: WeatherType }) {
  return (
    <div>
      <div style={{ fontFamily: 'var(--font-heading)', color: INK, letterSpacing: '0.15em', fontSize: '0.55rem', textTransform: 'uppercase', marginBottom: 4 }}>
        Weather Conditions
      </div>
...
export function LogPage({ entry, pageNum, total }: { entry: LogEntry; pageNum: number; total: number }) {
  ...
  return (
    ...
    >
      {/* Title */}
      <div style={{ textAlign: 'center', paddingBottom: 4, marginBottom: 5, borderBottom: `2px solid rgba(28,16,8,0.28)`, fontFamily: 'var(--font-heading)', fontSize: '0.62rem', letterSpacing: '0.2em', color: INK, textTransform: 'uppercase', flexShrink: 0 }}>
        Captain's Log Book
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', border: `1px solid rgba(28,16,8,0.28)`, marginBottom: 4, flexShrink: 0 }}>
        {[
          ...
          <div key={i} style={{ padding: '3px 6px', borderRight: i % 2 === 0 ? `1px solid rgba(28,16,8,0.28)` : 'none', borderBottom: i < 2 ? `1px solid rgba(28,16,8,0.28)` : 'none' }}>
            <div style={{ fontSize: '0.42rem', letterSpacing: '0.18em', textTransform: 'uppercase', fontFamily: 'var(--font-heading)', color: INK_FADED }}>{f.label}:</div>
            <div style={{ fontFamily: "'Caveat', cursive", fontSize: '0.82rem', color: INK, lineHeight: 1.25 }}>{f.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', border: `1px solid rgba(28,16,8,0.28)`, marginBottom: 4, flexShrink: 0 }}>
        {[
          ...
          <div key={i} style={{ padding: '2px 6px', gridColumn: i === 2 ? 'span 2' : undefined, borderRight: i === 0 ? `1px solid rgba(28,16,8,0.28)` : 'none', borderBottom: i < 2 ? `1px solid rgba(28,16,8,0.28)` : 'none' }}>
            <div style={{ fontSize: '0.42rem', letterSpacing: '0.15em', textTransform: 'uppercase', fontFamily: 'var(--font-heading)', color: INK_FADED }}>{f.label}:</div>
            <div style={{ fontFamily: "'Caveat', cursive", fontSize: '0.8rem', color: INK, lineHeight: 1.25 }}>{f.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ border: `1px solid rgba(28,16,8,0.28)`, marginBottom: 5, minHeight: 220, display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ padding: '2px 7px', borderBottom: `1px solid rgba(28,16,8,0.28)`, fontFamily: 'var(--font-heading)', fontSize: '0.42rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: INK, flexShrink: 0 }}>
          Log Entry — {entry.category}
        </div>
        <div
          ref={logEntryRef}
          className="log-ruled"
          style={{ padding: '2px 8px 4px 28px', fontFamily: "'Caveat', cursive", fontSize: '0.88rem', color: INK, lineHeight: `${LOG_LINE_HEIGHT}px`, position: 'relative' }}
        >
          <div style={{ position: 'absolute', top: 0, bottom: 0, left: 22, width: 1, background: 'rgba(139,26,26,0.35)', pointerEvents: 'none' }} />
          {wrappedLines.map((line, i) => (
            <div key={`${i}-${line}`} style={{ minHeight: `${LOG_LINE_HEIGHT}px`, position: 'relative' }}>
              <div
                style={{
                  position: 'absolute',
                  right: '100%',
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', borderTop: `1px solid rgba(28,16,8,0.22)`, paddingTop: 4, flexShrink: 0 }}>
        <div>
          <div style={{ fontSize: '0.42rem', letterSpacing: '0.15em', textTransform: 'uppercase', fontFamily: 'var(--font-heading)', color: INK_FADED }}>Logged By:</div>
          <div style={{ fontFamily: "'Caveat', cursive", fontSize: '0.82rem', color: INK }}>{entry.loggedBy}</div>
        </div>
...
export function CoverPage() {
  return (
    <div style={{ height: '100%', background: '#2e1806', backgroundImage: `radial-gradient(ellipse at 30% 70%, rgba(80,40,5,0.6) 0%, transparent 60%), radial-gradient(ellipse at 70% 30%, rgba(60,30,5,0.4) 0%, transparent 55%)`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px', borderRadius: '4px 0 0 4px' }}>
      <div style={{ border: '1px solid rgba(212,175,55,0.4)', padding: '18px 14px', width: '100%', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', gap: 6 }}>
...
export function BackCoverPage() {
  return (
    <div style={{ height: '100%', background: '#2e1806', backgroundImage: `radial-gradient(ellipse at 70% 30%, rgba(80,40,5,0.5) 0%, transparent 55%)`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', borderRadius: '0 4px 4px 0' }}>
